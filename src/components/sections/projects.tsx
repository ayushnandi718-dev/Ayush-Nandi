...
import { spawnNyanCat, default as NyanCat } from "../nyan-cat";
...
const ProjectsSection = () => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Intensity: faster typing = higher intensity
      if (timeSinceLast < 200) {
        keyCount.current = Math.min(keyCount.current + 1, 8);
      } else if (timeSinceLast < 500) {
        keyCount.current = Math.max(keyCount.current - 1, 1);
      } else {
        keyCount.current = 1;
      }

      const intensity = Math.min(keyCount.current / 3, 1);
      spawnNyanCat(Math.max(intensity, 0.3));
    };
  ...
  return (
    <SectionWrapper id="projects" className="max-w-7xl mx-auto md:min-h-[130vh] px-4">
      ...
      <div className="flex items-center justify-center gap-2 mb-8 px-4 py-2.5 rounded-full border border-border/60 bg-secondary/30 backdrop-blur-sm w-fit mx-auto">
        <Keyboard size={16} className="text-foreground/70" />
        <span className="text-sm font-medium text-foreground/80">
          Press any key on your keyboard for a surprise
        </span>
      </div>
